...
/**
 * ============================================================================
 *  HouseScene.vue —— 阶段 08：开关门与昼夜
 * ----------------------------------------------------------------------------
 *  累积内容（从 stage-01 到 stage-08）：
 *    [stage-01] 草地
 *    [stage-02] 地基平台 + 入口台阶
 ...
 *
 *  本阶段新增教学：
 *    1) requestAnimationFrame + THREE.Clock.getDelta() 帧率无关动画
 *    2) 门轴 pivot（Group 做门轴，门板偏移到一侧）+ lerp 旋转插值
 *    3) 键盘交互触发状态切换（O 开关门 / L 开关灯）
 *    4) 昼夜循环：阳光 DirectionalLight 颜色/intensity 时间驱动 + 环境光 +
 *       天空颜色 lerp，演示"关键帧"概念（用余弦函数在昼/夜两端插值）
 *
 *  动画系统设计：
 *    - clock = new THREE.Clock()，渲染循环中 getDelta() 取得帧间隔 dt
 *    - 门：doorState (0=关,1=开) 目标值，doorAngle 当前值，每帧 lerp 到目标
 *    - 灯：lampOn 目标布尔，lampIntensity 当前值，每帧 lerp 到 0 或 1
 *    - 昼夜：cycleTime 累加，对一天周期 (DAY_PERIOD 秒) 取模，归一化 t∈[0,1]
 *      再用余弦函数把 t 映射到光照强度，得到平滑昼/夜过渡
 * ============================================================================
 */
...
onMounted(() => {
  ...
  addHouse(); // 5~7. [stage-03/04/05] 房子（墙+门+屋顶+烟囱+地板+玻璃）
...
<template>
  ...
  <div style="position: relative; width: 100%; height: 100%">
    ...
    <div
      ...
      "
    >
      ⏱️ requestAnimationFrame + Clock.getDelta() · 帧率无关动画 · lerp 平滑过渡
    </div>
  </div>
</template>
